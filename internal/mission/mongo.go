...
func NewMongoStore(database *mongo.Database, now func() time.Time, id IDGenerator) *MongoStore {
	return &MongoStore{missions: database.Collection("missions"), audits: database.Collection("mission_audit"), costs: database.Collection("ai_cost_ledger"), jobs: database.Collection("processing_jobs"), now: now, id: id}
}
...
func (s *MongoStore) ClaimNextExport(ctx context.Context, now, leaseUntil time.Time) (ProcessingJob, ExportRequest, bool, error) {
	if _, err := s.jobs.UpdateMany(ctx, bson.M{"state": JobRunning, "leaseuntil": bson.M{"$lte": now}, "attempt": bson.M{"$gte": 3}}, bson.M{"$set": bson.M{"state": JobFailed, "updatedat": now, "lasterror": "maximum export attempts exceeded"}}); err != nil {
		return ProcessingJob{}, ExportRequest{}, false, err
	}
	filter := bson.M{"attempt": bson.M{"$lt": 3}, "$or": bson.A{bson.M{"state": JobQueued}, bson.M{"state": JobRunning, "leaseuntil": bson.M{"$lte": now}}}}
...
func (s *MongoStore) CompleteExport(ctx context.Context, id string, at time.Time) (ProcessingJob, error) {
	var stored storedProcessingJob
	err := s.jobs.FindOneAndUpdate(ctx, bson.M{"id": id, "state": bson.M{"$in": bson.A{JobQueued, JobRunning}}}, bson.M{"$set": bson.M{"state": JobSucceeded, "updatedat": at, "lasterror": ""}}, options.FindOneAndUpdate().SetReturnDocument(options.After)).Decode(&stored)
	if errors.Is(err, mongo.ErrNoDocuments) {
...
func (s *MongoStore) FailExport(ctx context.Context, id, message string, at time.Time) error {
	_, err := s.jobs.UpdateOne(ctx, bson.M{"id": id}, bson.M{"$set": bson.M{"state": JobFailed, "updatedat": at, "lasterror": message}})
	return err
}

func (s *MongoStore) GetExportJob(ctx context.Context, id string) (ProcessingJob, error) {
	var stored storedProcessingJob
	err := s.jobs.FindOne(ctx, bson.M{"id": id}).Decode(&stored)
	if errors.Is(err, mongo.ErrNoDocuments) {
