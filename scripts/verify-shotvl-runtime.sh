...
if rg -n 'kind: (Ingress|HTTPRoute)|type: (NodePort|LoadBalancer)' "$root/deploy/dev/shotvl"; then
  echo "ShotVL must not have a public route" >&2
  exit 1
fi

if rg -n '(ghp_|github_pat_|hf_[A-Za-z0-9]{16,}|api-key: [^<])' "$root/deploy/dev/shotvl" "$root/scripts/shotvl-runtime.sh"; then
